...
;	ボタン用のマクロ
	...
	tf.text_sample = 'このスピードでテキストが表示されます';
...
;	BGMミュート
	[config_button name="bgmvol,bgmvol_0" target="*vol_bgm_change" x="&tf.config_x[0]" y="&tf.config_y_bgm[0]" exp="tf.current_bgm_vol = 0; tf.config_num_bgm = 0"]
...
;	SEミュート
	[config_button name="sevol,sevol_0" target="*vol_se_change" x="&tf.config_x[0]" y="&tf.config_y_se[0]" exp="tf.current_se_vol = 0; tf.config_num_se = 0"]
...
;	テキスト表示速度のサンプルに使用するメッセージレイヤの設定
	[position layer="message1" left="20" top="530" width="920" height="90" margint="7" marginl="30" page="fore" visible="true" opacity="0"]
...
;	サンプルテキストを表示する
	[emb exp="tf.text_sample"]
		...
		[iscript]
		$(".current_span").css("color","#CCCCCC"); // サンプルテキストのフォントカラーを指定
		tf.system.backlog.pop(); // サンプルテキストを履歴から削除
		[endscript]

;	待ち時間をテキスト速度とサンプルの文字数に対応（消えるのが早すぎるときは後ろの数字を増やしてね）
